    ...
    public class EnemySpawner : MonoBehaviour
    {
        ...
        private void Spawning()
        {
            ...
            if (_timer >= SpawnInterval)
            {
                ...
                EnemyId randomEnemy = PickRandomEnemy();
                _enemyFactory.CreateEnemy(randomEnemy, spawnPosition, Quaternion.identity);
                _timer = 0;
            }
        }

        private EnemyId PickRandomEnemy()
        {
            float r = Random.value;
            if (r < 0.6f) return EnemyId.Walker;
            else if (r < 0.9f) return EnemyId.Skeleton;
            else return EnemyId.Boss;
        }
